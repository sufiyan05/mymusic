...
extension SignUpVM {
    ...
    func apiSignUp(_ email: String, vc: UIViewController) {
        
        let errMsg = self.validation(email)
        if errMsg != "" {
            vc.showAlert(strMessage: errMsg)
        } else {
            let vcOTPVarification = StoryboardScene.Authentication.otpVarificationVC.instantiate()
            vc.navigationController?.pushViewController(vcOTPVarification, animated: true)
        }
    }
}
